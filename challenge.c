...
void sparse_multiply(
    ...
    // Pass 1: walk A row-major, copy non-zeros into CSR buffers,
    // record each row's starting offset in row_ptrs.
    int k = 0;
    ...
    *out_nnz = k;
    ...
    // Pass 2: y = A * x using only the non-zeros we just extracted.
    for (int r = 0; r < rows; ++r) {
        double s = 0.0;
        for (int j = row_ptrs[r]; j < row_ptrs[r + 1]; ++j) {
            s += values[j] * x[col_indices[j]];
        }
        y[r] = s;
    }
}
